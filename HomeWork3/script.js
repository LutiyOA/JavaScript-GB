...
Создать функцию, которая высчитывает 13% от данного числа и выводит 
...
const pow = (number, rank) => number ** rank;

console.log(pow(2, 3) + pow(3, 3));

//--------------------------------------------------------------------
// задание 2
const salary = +prompt('Введите зарплату:');

if (isFinite(salary)) printSalaryWithoutTax(salary);
else alert('Введено некорректное значение!');

function printSalaryWithoutTax(salary) {
    alert(`Размер заработной платы за вычетом налогов равен ${salary * .87}`);
}
...
alert(`Максимум = ${max(number1, number2, number3)}`);
